<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="./styles/one.css">
    <title>Document</title>
</head>
<body>
    <header>
        <div class="header-text">
            <h1>Расписание</h1>
        </div>
    </header>

    <main>
        <section class="welcom">
            <h2>Расписание на неделю:</h2>
            <table border="1" rules="groups">
                <tr class="Teacher">
                    <th width='20%'>День недели</th>
                    <th width="15%">Урок</th>
                    <th>Предмет</th>
                    <th width="20%">Кабинет</th>
                </tr>
    
                <colgroup>
                    <tr class='lo'>
                        <td rowspan="8">Понедельник</td>
                        <td>0 урок</td>
                        <td>Разговоры о важном</td>
                        <td>330</td>
                    </tr>
                </colgroup>
                
                <tr>
                    <td>1 урок</td>
                    <td>Английский язык</td>
                    <td>324/326</td>
                </tr>
                <tr>
                    <td>2 урок</td>
                    <td>Алгебра</td>
                    <td>229</td>
                </tr>
                <tr>
                    <td>3 урок</td>
                    <td>История Ставрополья</td>
                    <td>125</td>
                </tr>
                <tr>
                    <td>4 урок</td>
                    <td>География</td>
                    <td>327</td>
                </tr>
                <tr>
                    <td>5 урок</td>
                    <td>Биология</td>
                    <td>121</td>
                </tr>
                <tr>
                    <td>6 урок</td>
                    <td>Физика</td>
                    <td>223</td>
                </tr>
                <tr>
                    <td>7 урок</td>
                    <td>Химия</td>
                    <td>323</td>
                </tr>
                <colgroup>
                    <tr class='lo'>
                        <td rowspan="7">Вторник</td>
                        <td>1 урок</td>
                        <td>Технология</td>
                    </tr>
                </colgroup>
                
        
                <tr>
                    <td>2 урок</td>
                    <td>Технология</td>
                </tr>
        
                <tr>
                    <td>3 урок</td>
                    <td>Алгебра</td>
                    <td>229</td>
                </tr>
        
                <tr>
                    <td>4 урок</td>
                    <td>Теория Вероятности</td>
                    <td>229</td>
                </tr>
        
                <tr>
                    <td>5 урок</td>
                    <td>Физ-ра</td>
                </tr>
        
                <tr>
                    <td>6 урок</td>
                    <td>ОБЖ</td>
                    <td>330</td>
                </tr>
        
                <tr>
                    <td>7 урок</td>
                    <td>Литература</td>
                    <td>127</td>
                </tr>
                <colgroup>
                    <tr class='lo'>
                        <td rowspan="7">Среда</td>
                        <td>1 урок</td>
                        <td>Английский язык</td>
                        <td>324/326</td>
                    </tr>
                </colgroup>
                
        
                <tr>
                    <td>2 урок</td>
                    <td>История</td>
                    <td>330</td>
                </tr>
        
                <tr>
                    <td>3 урок</td>
                    <td>Музыка</td>
                    <td>107</td>
                </tr>
        
                <tr>
                    <td>4 урок</td>
                    <td>Алгебра</td>
                    <td>229</td>
                </tr>
        
                <tr>
                    <td>5 урок</td>
                    <td>Русский язык</td>
                    <td>127</td>
                </tr>
        
                <tr>
                    <td>6 урок</td>
                    <td>Химия</td>
                    <td>323</td>
                </tr>
        
                <tr>
                    <td>7 урок</td>
                    <td>Физ-ра</td>
                </tr>
                <colgroup>
                    <tr class='lo'>
                        <td rowspan="7">Четверг</td>
                        <td>1 урок</td>
                        <td>История</td>
                        <td>332</td>
                    </tr>
                </colgroup>
                
        
                <tr>
                    <td>2 урок</td>
                    <td>Геометрия</td>
                    <td>229</td>
                </tr>
        
                <tr>
                    <td>3 урок</td>
                    <td>География</td>
                    <td>327</td>
                </tr>
        
                <tr>
                    <td>4 урок</td>
                    <td>Русский язык</td>
                    <td>127</td>
                </tr>
        
                <tr>
                    <td>5 урок</td>
                    <td>Литература</td>
                    <td>127</td>
                </tr>
        
                <tr>
                    <td>6 урок</td>
                    <td>Биология</td>
                    <td>121</td>
                </tr>
        
                <tr>
                    <td>7 урок</td>
                    <td>Профминимум</td>
                    <td>330</td>
                </tr>
                
    
                <colgroup>
                    <tr>
                        <td rowspan="6">Пятница</td>
                        <td>1 урок</td>
                        <td>Физика</td>
                        <td>223</td>
                    </tr>
                </colgroup>
                
        
                <tr>
                    <td>2 урок</td>
                    <td>Геометрия</td>
                    <td>229</td>
                </tr>
        
                <tr>
                    <td>3 урок</td>
                    <td>Обществознание</td>
                    <td>330</td>
                </tr>
        
                <tr>
                    <td>4 урок</td>
                    <td>Русский язык</td>
                    <td>127</td>
                </tr>
        
                <tr>
                    <td>5 урок</td>
                    <td>Английский язык / Информатика</td>
                    <td>326/201</td>
                </tr>
        
                <tr>
                    <td>6 урок</td>
                    <td>Английский язык / Информатика</td>
                    <td>324/201</td>
                </tr>
            </table>
        </section>
    </main>

    
    <footer>
        <div class="footer-img">
            <a href="index.html"><img class="fot_img" src="svgs/home.svg" alt=""></a>
            <a href=""><img class="fot_img" src="svgs/raspisanie.svg" alt=""></a>
            <a href=""><img class="fot_img" src="svgs/chat.svg" alt=""></a>
            <a href="Group.html"><img class="fot_img" src="svgs/user.svg" alt=""></a>
        </div>
    </footer>
</body>
</html>